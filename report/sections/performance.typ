...
== SpeedUp
    ...
    The first implementation sought to assess the speedup using a single dataset and multiple CPU counts (1, 2, 4, 16, 32, and 64) and thread counts (1, 2, 4).
...
=== SpeedUp Results and Discussion
    ...
    This is expected because of the added overhead. Interestingly, the computation_time is almost identical to total_time; this suggests that the setup/teardown of the MPI environment is adding a fixed cost to the parallel logic.
...
=== Further Analysis
    ...
    As mentioned above, strong scaling occurs as the runtime falls as the number of compute resources (cores, MPI ranks, threads…) increases; however, this is only true when $"np" = 1 → 4, "threads" <= 4$, since each iteration has a ~30% reduction when doubling cores.
    This is consistent except for np=2, threads=4, where saturation starts to show as we see greater results compared with the 2-thread case, most probably a bad run due to some network constraint. The behavior then deviates back to $"np" = 16 → 32, "threads" <= 2$, where speedup still occurs even at a more inclined rate, but only lasts for 1 iteration of threads since the tendency continues to grow at 1 thread, which indicates OpenMP is no longer in use.
...
== Parallelization Implementation
    ...
    Now that we have proven that the code is able to achieve Strong Scaling, the second implementation assesses parallelization prioritizing a more diverse dataset (256, 512, 1024 and 2048) and a simpler configuration of CPUs (16, 32, 64, 128) and thread counts (2, 4, 6). 
    Conditions such as memory or placement location were kept the same.
...
=== Parallelization Implementation Results and Discussion
    ...
    However, if we analyze the implementation of OpenMP @openmp_conv, we can see a different result. When using $"thread"=2$, 128 threads take a while to converge; this behavior seems to be constant regardless of the total number of threads. This means that at np=128 we start to see saturation. 
