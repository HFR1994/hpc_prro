 ...
 */
double initialize_params(const char *dataset_path, double *X, double *fitness, double *roosting_site,
    ...
    read_dataset_csv(dataset_path, X, pop_size, features);

    // Make sure all vector positions are inside the bounds otherwise set the min/max
    check_bounds(X, pop_size, features, lower_bound, upper_bound);

    // Evaluate Griewank function
    objective_function(X, fitness, pop_size, features);
    ...
    // Instead of adding more variables to the method we just compute the value and assgiugn
