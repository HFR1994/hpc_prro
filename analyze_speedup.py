...
def extract_timings(log_path):
    ...
    try:
        ...
        if total_match:
            timings['total_time'] = float(total_match.group(1))

        # Extract computation_time
        comp_match = re.search(r'computation_time:\s+([\d.]+)', content)
        if comp_match:
            timings['computation_time'] = float(comp_match.group(1))
...
def main():
    ...
    # Configuration
    logs_dir = 'logs/final/output'  # Change this to your logs directory
